...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29475;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/lostpetflyer") {
      try {
        const { personality } = await readBody(req);
        if (!personality || !personality.trim()) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: "Please fill in the field first" }));
          return;
        }
        const result = await generate(modelId, personality.trim());
